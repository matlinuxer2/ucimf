...
LC_CTYPE=zh_TW.UTF-8 gdb -batch -x ${BUILD}/debug/trace.gdb ${BUILD}/bin/fbterm >> error.log 2>&1
...
then
	...
	cd ${BUILD}
	LC_CTYPE=zh_TW.UTF-8 gdb -batch -x ${BUILD}/debug/trace.gdb ${BUILD}/bin/console01 >>error.log 2>&1

	echo "===Run again message===" | tee -a error.log
	LC_CTYPE=zh_TW.UTF-8 ${BUILD}/bin/console01 >>error.log 2>&1
	if (( $? )) 
	then
	...
	cd ${BUILD}
	LC_CTYPE=zh_TW.UTF-8 gdb -batch -x ${BUILD}/debug/trace.gdb ${BUILD}/bin/console02 >>error.log 2>&1

	echo "===Testing console case 03===" | tee -a error.log
	cd ${BUILD}
	FONT_PATH=${BUILD}/share/ucimf/unifont.pcf gdb -batch -x ${BUILD}/debug/trace.gdb ${BUILD}/bin/console03 >>error.log 2>&1
fi
